...
Check cache – if a response for the key exists and is not expired, return it immediately.

Acquire distributed lock – prevents other threads/instances from entering the same critical section.

Double‑check cache – another request might have finished while we waited for the lock.

Execute business logic – only if no cached response exists.

Save response – store the result with the configured TTL.

Release lock – allow other requests to use the cached result.
...
LockExpiry	00:00:10	Maximum lifetime of a lock (prevents dead locks).
Redis Options
Property	Default	Description
